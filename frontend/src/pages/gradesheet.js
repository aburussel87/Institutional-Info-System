...
export default function Gradesheet() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (data.result.current_semester) {
  ...
  const semesterOptions = student.levelTerms || [];

  const courses = selectedTerm && student.grades && student.grades[selectedTerm]
    ? student.grades[selectedTerm]
    : [];
  ...
  let weightedSum = 0;
  semesterOptions.forEach((termName) => {
    (student.grades[termName] || []).forEach((c) => {
      totalCreditsTillNow += c.credit;
      weightedSum += c.credit * c.gpa;
    });
  });
  const totalCGPA = totalCreditsTillNow > 0 ? weightedSum / totalCreditsTillNow : 0;
      ...
      {selectedTerm && (
        <section id="gradesheet" style={{ marginTop: "3rem" }}>
          <div className="text-center mb-4">
            ...
            <p><strong>Selected Level-Term:</strong> {selectedTerm}</p>
          </div>
          ...
          <div>
            <strong>Total Credit Hours Completed:</strong> {totalCreditsTillNow.toFixed(1)}<br />
